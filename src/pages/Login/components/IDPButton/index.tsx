...
const IDPButton = ({
  ...
  const buttonContent = (
    <>
      {loading ? (
        ...
        <span className={styles.iconPlaceholder}>{displayName.charAt(0)}</span>
      )}
      {strategy.length > 1 && <DownOutlined className={styles.arrow} />}
    </>
  );

  // 如果没有 strategy 或只有一个，直接点击
  ...
  return (
    <Dropdown
      menu={{ items: menuItems, className: styles.dropdown }}
      trigger={['click']}
      disabled={disabled || loading}
    >
      <button
